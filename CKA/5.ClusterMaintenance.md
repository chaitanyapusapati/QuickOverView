...
- Run the etcdctl snapshot restore command
 ...
 ### **or**
 ```
 $ ETCDCTL_API=3 etcdctl snapshot restore --endpoints=https://172.17.0.56:2379 --cacert=/etc/kubernetes/pki/etcd/ca.crt --cert=/etc kubernetes/pki/etcd/server.crt --key=/etc/kubernetes/pki/etcd/server.key --data-dir="/var/lib/etcd-backup" --initial-cluster="controlplane=https://172.17.0.56:2380" --name="controlplane" --initial-advertise-peer-urls="https://172.17.0 56:2380" --initial-cluster-token="etcd-cluster-1" /opt/snapshot-pre-boot.db
 ```
